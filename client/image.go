	...
	for _, layer := range layers {
		...
		if unpacked {
			...
			if _, err := cs.Update(ctx, cinfo, "labels."+labels.LabelUncompressed); err != nil {
				// Lazy-ingest layers (org.erofs.chunk-index.range) live in the
				// indexed content store, not the regular content store. The
				// label update is a hint for future eager unpacks; skip it
				// silently when the blob is absent from the regular store.
				if !errdefs.IsNotFound(err) {
					return err
				}
			}
