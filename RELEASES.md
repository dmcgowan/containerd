...
longer period or extend the support period as needed. These branches will accept bug reports and
...
and any branch may become an _LTS_ branch after its initial release. The owners of the _LTS_ branch
may step down or be replaced by another maintainer at any time if they can no longer support the
branch. If no maintainers volunteer to the own the branch, the branch will either be reverted back
to non-_LTS_ support timeframe is less than a year old or end of life after 6 months with the
maintainers providing extended support.
