...
func init() {
	registry.Register(&plugin.Registration{
		...
		InitFn: func(ic *plugin.InitContext) (any, error) {
			...
			// Wire the indexed content store when it is available.
			// This enables UnpackConfiguration.Lazy for formats that support it
			// (currently: EROFS layers with org.erofs.chunk-index.range).
			idxRaw, err := ic.GetSingle(plugins.ContentIndexPlugin)
			if err != nil && !errors.Is(err, plugin.ErrPluginNotFound) {
				return nil, fmt.Errorf("transfer: look up content index plugin: %w", err)
			}
			if idxRaw != nil {
				if is, ok := idxRaw.(local.LazyIndexStore); ok {
					lc.IndexStore = is
				}
			}

			// Wire the cache warmer when available so lazy pulls trigger
			// background chunk population.  Optional: cache is only
			// registered on Linux where the kernel mount path exists.
			cacheRaw, err := ic.GetSingle(plugins.CachePlugin)
			if err != nil && !errors.Is(err, plugin.ErrPluginNotFound) {
				return nil, fmt.Errorf("transfer: look up cache plugin: %w", err)
			}
			if cacheRaw != nil {
				if w, ok := cacheRaw.(local.LazyCacheWarmer); ok {
					lc.CacheWarmer = w
				}
			}
