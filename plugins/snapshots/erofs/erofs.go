...
type SnapshotterConfig struct {
	...
	// Cannot be used with scratchFile
	defaultSize int64
	// scratchFile is a file path to a pre-created sparse file for writable layers
	scratchFile string
	// scratchFSType is the filesystem type of the scratchFile, defaults to ext4
	scratchFSType string
}
...
func WithScratchFile(name, fstype string) Opt {
	return func(config *SnapshotterConfig) {
		config.scratchFile = name
		if fstype != "" {
			config.scratchFSType = fstype
		} else {
			config.scratchFSType = "ext4"
		}
	}
}
...
func NewSnapshotter(root string, opts ...Opt) (snapshots.Snapshotter, error) {
	...
	return &snapshotter{
		...
		scratchFile:     config.scratchFile,
		scratchFSType:   config.scratchFSType,
		defaultWritable: config.defaultSize,
		blockMode:       config.defaultSize > 0 || config.scratchFile != "",
	}, nil
}
...
func (s *snapshotter) prepareDirectory(ctx context.Context, snapshotDir string, kind snapshots.Kind) (string, error) {
	...
	log.G(ctx).WithField("blockmode", s.blockMode).WithField("kind", kind).Debugf("Prepare snapshot with snapshot %s", td)
	if kind == snapshots.KindActive {
		if s.blockMode {
			log.G(ctx).WithField("scratch", s.scratchFile).Debugf("Creating writable layer for snapshot %s", td)
			if s.scratchFile != "" {
				// Use the provided sparse file as the writable layer backing file
				if _, err := os.Stat(s.scratchFile); err != nil {
					return td, fmt.Errorf("failed to access scratch file %s: %w", s.scratchFile, err)
				}
				if err := fs.CopyFile(filepath.Join(td, "layer.ext4"), s.scratchFile); err != nil {
					return td, fmt.Errorf("failed to copy scratch file %s: %w", s.scratchFile, err)
				}
			} else {
				// TODO: Get size from snapshot labels
				if err := createWritableImage(filepath.Join(td, "layer.ext4"), s.defaultWritable); err != nil {
...
func (s *snapshotter) commitBlock(ctx context.Context, layerBlob string, id string) error {
	...
	if cerr := convertDirToErof(ctx, layerBlob, filepath.Join(s.upperPath(id), "upper")); cerr != nil {
