	...
	registry.Register(&plugin.Registration{
		...
		InitFn: func(ic *plugin.InitContext) (interface{}, error) {
			...
			go func() {
				...
				// Read all mount state and sync with DB
				t.Commit()

			}()

			md.(*metadata.DB).RegisterCollectibleResource(metadata.ResourceMount, mm)
			return mm, nil
		},
	})
}
...
func (mm *mountManager) Activate(ctx context.Context, name string, mounts []mount.Mount, opts ...mount.ActivateOpt) (info mount.ActivationInfo, retErr error) {
	...
	for i := range mounts {
		...
		// Look for handlers by type to increment firstSystemMount pointer
	}
	...
	if err := mm.db.Update(func(tx *bolt.Tx) error {
		...
		nsbkt, err := tx.CreateBucketIfNotExists([]byte(namespace))
		if err != nil {
			return err
		}
		mbkt, err := nsbkt.CreateBucketIfNotExists([]byte("mounts"))
		if err != nil {
			return err
		}
	...
	defer func() {
		if retErr != nil {
			if err := mm.db.Update(func(tx *bolt.Tx) error {
				...
				if v1bkt == nil {
					return fmt.Errorf("missing bucket", errdefs.ErrUnknown)
				}

				nsbkt := tx.Bucket([]byte(namespace))
				if nsbkt == nil {
					return fmt.Errorf("missing bucket", errdefs.ErrUnknown)
				}
				...
				log.G(ctx).WithField("name", name).Errorf("failed to rollback")
			}
		}
	}()
