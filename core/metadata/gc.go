...
// (for example an index blob that references thousands of content chunks) can
...
type gcContext struct {
	...
	// forward references lazily (see collectionWithReferences).  Consulted by
	// references() after the built-in core type switch.
...
func startGCContext(ctx context.Context, collectors map[gc.ResourceType]Collector) *gcContext {
	...
	if len(collectors) > 0 {
		...
		for rt, collector := range collectors {
			...
			// Register the lazy forward-reference emitter, if implemented.
...
func (c *gcContext) references(ctx context.Context, tx *bolt.Tx, node gc.Node, fn func(gc.Node)) error {
	...
	// Collectible resource types that emit forward references lazily
	// (see collectionWithReferences) are handled here, after the built-in
	// core types.  This lets a collector such as the indexed-content store
	// reference all of a blob's content chunks on demand, without having to
	// pre-compute and hold every edge in backRefs for the whole pass.
