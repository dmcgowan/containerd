...
func (mm *mountManager) Activate(ctx context.Context, name string, mounts []mount.Mount, opts ...mount.ActivateOpt) (info mount.ActivationInfo, retErr error) {
	...
	shouldPrepare := func(p string, t string) bool {
		p = p + "/*"
	...
	prepares := map[string]mount.Transformer{
	...
	for i := range mounts {
		...
		// Check is the source needs preparation, any prepare operation requires
		// mounting with the mount manager.
		for prepare, mt, ok := strings.Cut(mountType, "/"); ok; prepare, mt, ok = strings.Cut(mountType, "/") {
			if tr, ok := prepares[prepare]; ok {
				...
				log.G(ctx).Warnf("unknown transform %q for mount %v", prepare, mounts[i])
				break
			}
		}
