...
const (
	...
	//
	// The dm-verity data is stored WITHOUT an on-disk superblock: the
	// bytes at [hash_offset, end) are the merkle tree itself, with no
	// preceding veritysetup superblock block.  Consumers MUST activate
	// the device in no-superblock mode (veritysetup --no-superblock) and
	// supply the format parameters out-of-band from these annotations:
	// hash algorithm is always SHA-256, hash type 1, no salt, and the
	// data/hash block size is org.erofs.dmverity.block_size (default
	// 4096).  The number of data blocks is hash_offset / block_size.
	AnnotationDmVerityHashOffset = "org.erofs.dmverity.hash_offset"

	// AnnotationDmVerityRootDigest is the merkle tree's root digest,
	// formatted "sha256:<hex>".  This is the value passed to the kernel
	// dm-verity target at activation; it is NOT stored on-disk (there is
	// no superblock).
...
// IsZstdMediaType reports whether mediaType identifies a +zstd EROFS layer
// variant (i.e. one whose on-blob chunks are wrapped in zstd frames).
func IsZstdMediaType(mediaType string) bool {
	switch mediaType {
	case MediaTypeEROFSZstd, MediaTypeEROFSLayerZstd:
		return true
	}
	return false
}
