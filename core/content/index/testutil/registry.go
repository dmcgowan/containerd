...
func (r *MemRegistry) handleManifest(w http.ResponseWriter, req *http.Request, path, method string) {
	// path: /v2/<repo>/manifests/<ref>  (or "/<repo>/..." for clients that
	// do not prefix /v2/).  Normalise to the bare repo by stripping the
	// leading slash and any /v2/ prefix so push and fetch agree on the
	// map key regardless of which client style produced the URL.
	parts := strings.SplitN(path, "/manifests/", 2)
	if len(parts) != 2 {
		http.Error(w, "bad request", http.StatusBadRequest)
		return
	}
	repo := strings.TrimPrefix(parts[0], "/")
	repo = strings.TrimPrefix(repo, "v2/")
	ref := parts[1]
...
// PutBlob inserts raw bytes directly into the registry without going through
// the HTTP upload protocol.  The digest is computed from data.
// This is intended for test fixtures that need to pre-populate the registry.
func (r *MemRegistry) PutBlob(data []byte) digest.Digest {
	dgst := digest.FromBytes(data)
	r.mu.Lock()
	r.blobs[dgst.String()] = data
	r.mu.Unlock()
	return dgst
}

// PutManifest inserts a manifest directly under repo/ref (and also by digest).
// mediaType sets the Content-Type returned for GET requests.
func (r *MemRegistry) PutManifest(repo, ref string, data []byte, mediaType string) digest.Digest {
	dgst := digest.FromBytes(data)
	if mediaType == "" {
		mediaType = "application/vnd.oci.image.manifest.v1+json"
	}
	...
	r.manifests[repo][ref] = data
	r.manifests[repo][dgst.String()] = data
	r.blobs[dgst.String()] = data
	r.mediaTypes[dgst.String()] = mediaType
	r.mu.Unlock()
	return dgst
