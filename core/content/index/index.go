...
type LazyProvider interface {
	...
	// FillBatch fetches a CONTIGUOUS run of chunks in a single provider
	// Fetch call and writes each chunk to the content store under its
	// per-chunk hash.
	//
	// chunkIdxs MUST be sorted ascending and physically contiguous on
	// the blob (every chunk's OnBlobEnd equals the next chunk's
	// OnBlobStart).  Implementations MAY reject (or silently fall back
	// to per-chunk fills) if the run is not contiguous; the cache layer
	// guarantees contiguity by construction.
	//
	// Implementation semantics mirror FillChunk for each contained
	// chunk: per-chunk hash verification, idempotent re-entry, and
	// per-chunk content-store ingest.  The single network round-trip
	// is the only difference.
	//
	// Concurrent FillBatch and FillChunk calls coalesce per-chunk: a
	// chunk being filled by a peer batch causes this call to wait
	// rather than re-fetch.
	//
	// priority is forwarded verbatim to p.Fetch.
	FillBatch(ctx context.Context, dgst digest.Digest, chunkIdxs []int, p ByteProvider, priority Priority) error
}
...
type ByteProvider interface {
	...
	// Fetch returns `length` bytes of blob `desc` starting at byte
	// offset `off` (i.e. the half-open interval [off, off+length) of
	// the original blob).
	//
	// The provider sees only a byte range — it has no concept of
	// "chunks", "frames", or "compressed".  The caller is responsible
	// for choosing the right byte range to fetch (one chunk's
	// compressed frame, a coalesced multi-chunk range, the
	// chunk-index trailer, etc.), for decompression, and for hash
	// verification of the returned bytes.
	//
	// priority governs queue ordering: PriorityForeground requests
	// bypass any background prefetch queue and are dispatched
	// immediately within the concurrency budget.
	//
	// Implementations MUST honour ctx cancellation and return promptly.
	Fetch(ctx context.Context, desc ocispec.Descriptor, off, length int64, priority Priority) (io.ReadCloser, error)
}
...
type Info struct {
	...
	// UncompressedSize is the total uncompressed size of the blob (i.e.
	// the sum of every chunk's uncompressed length).  For raw (non-zstd)
	// layers this equals Size minus the chunk-index trailer length.  For
	// +zstd layers it is the size of the decompressed EROFS image, which
	// is what the cache materialises on disk so the kernel can mount it.
