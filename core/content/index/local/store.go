...
type Store struct {
	...
	// blobRemover, when set, is invoked by the GC collector for each indexed
	// blob digest that becomes unreferenced in every namespace.  The sparse-
	// file cache backend registers its Remove here so on-disk cache files are
	// reclaimed together with the blob's index metadata.  Set once at startup
	// via SetBlobRemover before any GC runs.
	blobRemover func(digest.Digest) error
}

// SetBlobRemover registers fn to be called during GC for each indexed blob
// digest that is no longer referenced in any namespace.  Intended for the
// cache backend to delete its on-disk files.  Must be called before the GC
// scheduler starts (i.e. at plugin init time); not safe for concurrent use
// with an in-flight collection.
func (s *Store) SetBlobRemover(fn func(digest.Digest) error) {
	s.blobRemover = fn
}
