...
func TestRoundTrip_ConvertPushPullUnpack(t *testing.T) {
	...
	// chunked.Build now writes the blob to an io.Writer and only
	// returns metadata; capture the bytes in a buffer so the rest
	// of the round-trip test (which compares blob bytes end-to-end)
	...
	resultBlob := blobBuf.Bytes()
	blobDigest := digest.FromBytes(resultBlob)
	t.Logf("built blob: size=%d chunks=%d", len(resultBlob), len(result.Chunks))
	...
	// Build leaves Digest empty (it streams to the writer and lets
	// the caller compute the digest from the writer); fill it in
	// from the captured buffer.
	layerDesc.Digest = blobDigest
	layerDesc.Size = int64(len(resultBlob))
	pushWriter, err := pusher.Push(ctx, layerDesc)
	if err != nil {
		t.Fatalf("Push layer: %v", err)
	}
	if _, err := pushWriter.Write(resultBlob); err != nil {
	...
	if ra.Size() != int64(len(resultBlob)) {
		t.Fatalf("assembled reader size %d != blob size %d", ra.Size(), len(resultBlob))
	}

	got := make([]byte, len(resultBlob))
	if _, err := ra.ReadAt(got, 0); err != nil && err != io.EOF {
		t.Fatalf("ReadAt: %v", err)
	}
	if !bytes.Equal(got, resultBlob) {
		for i := range resultBlob {
			if got[i] != resultBlob[i] {
				t.Fatalf("blob mismatch at offset %d: got 0x%02x want 0x%02x", i, got[i], resultBlob[i])
			}
		}
	}
	...
	for i, chunkRef := range result.Chunks {
		...
		if !bytes.Equal(buf[:n], resultBlob[chunkRef.OnBlobStart:chunkRef.OnBlobEnd]) {
...
func TestRoundTrip_UnpackSequentialStream(t *testing.T) {
	...
	descriptor.Digest = blobDigest
	descriptor.Size = int64(len(resultBlob))

	// Ingest.
	w, err := idxStore.Writer(ctx, content.WithRef("unpack-test"), content.WithDescriptor(descriptor))
	if err != nil {
		t.Fatal(err)
	}
	io.Copy(w, bytes.NewReader(resultBlob))
	if err := w.Commit(ctx, int64(len(resultBlob)), descriptor.Digest); err != nil {
		t.Fatal(err)
	}

	// Get the assembled reader.
	ra, err := idxStore.ReaderAt(ctx, descriptor)
	if err != nil {
		t.Fatal(err)
	}
	defer ra.Close()
	...
	if copied != int64(len(resultBlob)) {
		t.Fatalf("copied %d bytes, want %d", copied, len(resultBlob))
	}
	if !bytes.Equal(buf.Bytes(), resultBlob) {
	...
	if h.Digest() != descriptor.Digest {
		t.Fatalf("sequential digest mismatch: got %s want %s", h.Digest(), descriptor.Digest)
	}
