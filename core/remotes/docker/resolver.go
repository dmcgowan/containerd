...
// (e.g. response header timeouts, mid-stream EOFs). To match the response-status
// retry policy for 5xx errors, retries are only attempted on the last host so
// mirror fallback semantics are preserved.
func (r *request) doWithTransportRetries(ctx context.Context, attempts *int, lastHost bool) (resp *http.Response, err error) {
	for *attempts > 0 {
		resp, err = r.do(ctx)
		*attempts--
		if err == nil {
		...
		if !lastHost || !isTransientTransportErr(ctx, err) {
			return nil, err
		}
...
// retrying. Context cancellation is intentionally not considered transient.
func isTransientTransportErr(ctx context.Context, err error) bool {
	if ctx.Err() != nil {
		return false
	}
