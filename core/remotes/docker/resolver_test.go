...
func TestIsTransientTransportErr(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "net timeout", ctx: context.Background(), err: fakeTimeoutErr{}, want: true},
		{name: "wrapped net timeout", ctx: context.Background(), err: fmt.Errorf("wrapped: %w", fakeTimeoutErr{}), want: true},
		{name: "io.EOF", ctx: context.Background(), err: io.EOF, want: true},
		{name: "io.ErrUnexpectedEOF", ctx: context.Background(), err: io.ErrUnexpectedEOF, want: true},
		{name: "wrapped io.EOF", ctx: context.Background(), err: fmt.Errorf("wrapped: %w", io.EOF), want: true},
		{name: "generic error not transient", ctx: context.Background(), err: errors.New("nope"), want: false},
		{name: "context canceled not transient", ctx: context.Background(), err: context.Canceled, want: false},
		{name: "context deadline exceeded not transient", ctx: context.Background(), err: context.DeadlineExceeded, want: false},
		{name: "canceled ctx suppresses retry", ctx: canceledCtx, err: fakeTimeoutErr{}, want: false},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := isTransientTransportErr(tc.ctx, tc.err)
			if got != tc.want {
...
func TestDoWithTransportRetries(t *testing.T) {
	...
	t.Run("non-transient error is not retried", func(t *testing.T) {
		...
		if !strings.Contains(err.Error(), "connection refused") {
			t.Errorf("expected wrapped %q, got %v", nonTransient, err)
		}
		if calls != 1 {
			t.Errorf("expected 1 call, got %d", calls)
		}
	...
	t.Run("context cancellation during backoff", func(t *testing.T) {
		...
		// Cancel after the first attempt fails and the backoff sleep starts.
		go func() {
