...
// containerd queued after reading the result.
...
// SetDetachListener stores l so that serveListener can reuse it without
// removing and re-creating the socket. The socket must already be bound.
// Storing it before stdout is closed ensures containerd can connect the
// instant it reads the bootstrap result.
...
//
// For the AF_UNIX path, if Detachable.Detach pre-bound a listener via
// SetDetachListener, it is returned directly so that connections queued
// while the "start" process was still running are not lost.
func serveListener(path string, fd uintptr) (net.Listener, error) {
	...
	if socketPath, ok := strings.CutPrefix(path, "unix://"); ok {
		// Detachable.Detach pre-bound this listener before closing
		// stdout, so it already exists when containerd dials. Reuse it
		// rather than rebinding, which would orphan queued connections.
		if l := takeDetachListener(); l != nil {
			log.L.WithField("socket", socketPath).Debug("serving ttrpc on pre-bound AF_UNIX socket (Detachable)")
			return l, nil
		}

		// Normal AF_UNIX path: daemon creates its own socket.
...
func detachConsole() error {
	r, _, err := windows.NewLazySystemDLL("kernel32.dll").NewProc("FreeConsole").Call()
	if r == 0 {
