...
// same chunk result in a single provider fetch.
func TestLazyFillChunkCoalescing(t *testing.T) {
	...
	// Fire 10 concurrent FillChunk(0) calls.
	...
	t.Logf("FillChunk ×10 concurrent for same chunk: %d provider Fetch calls", fetches)
	// Coalescing means 1 fetch (or 2 if a second goroutine races through
	// before the content-store entry is visible). Accept ≤ 3.
	if fetches > 3 {
		t.Errorf("expected ≤3 Fetch calls with coalescing, got %d", fetches)
	}
}
...
// TestLazyCacheBitmapRestart verifies that the on-disk bitmap lets the cache
// resume a partially-filled blob after a simulated daemon restart.
func TestLazyCacheBitmapRestart(t *testing.T) {
	...
	// First activation: fill half the chunks.
	...
	// Count how many chunks were missing at first activation.
	missing1, _ := store.MissingChunks(ctx, lb.desc.Digest)
	t.Logf("After partial fill: %d/%d chunks still missing", len(missing1), numChunks)

	// Simulate restart: create fresh cache instance.
	...
	missing2, _ := store.MissingChunks(ctx, lb.desc.Digest)
	t.Logf("After restart, MissingChunks: %d (should still be %d)", len(missing2), len(missing1))

	if err := h2.EnsureAll(ctx); err != nil {
		t.Fatalf("EnsureAll after restart: %v", err)
	}
	missing3, _ := store.MissingChunks(ctx, lb.desc.Digest)
	if len(missing3) != 0 {
		t.Errorf("expected 0 missing after EnsureAll, got %d", len(missing3))
	}
	t.Logf("EnsureAll after restart: complete ✓")
}
...
// TestLazyLoopMount tests the full loop-mount path (requires root + erofs kernel module).
func TestLazyLoopMount(t *testing.T) {
	if os.Getuid() != 0 {
	...
	// This test would build a real EROFS filesystem image, lazily ingest it,
	// fill chunks, attach the cache, EnsureAll, and loop-mount the sparse file.
	// For now it verifies that the backing file path from the cache is a
	// regular file on disk (loop-mount wiring is in the mount manager plugin).
	store, cs := newLazyStore(t)
	lb := newLazyBlob(2, 1024)
	p := &fakeBP{data: lb.data}

	if err := store.WriteLazy(ctx, "loop-ref", lb.desc, p); err != nil {
		t.Fatalf("WriteLazy: %v", err)
	}
	for i := 0; i < 2; i++ {
		if err := store.FillChunk(ctx, lb.desc.Digest, i, p, contentindex.PriorityForeground); err != nil {
	...
	if _, err := os.Stat(backingFile); err != nil {
		t.Fatalf("backing file %s: %v", backingFile, err)
	}
	t.Logf("Backing file ready for loop mount: %s ✓", backingFile)
}
...
func (f *fakeBP) Fetch(_ context.Context, _ ocispec.Descriptor, c contentindex.ChunkRef, _ contentindex.Priority) (io.ReadCloser, error) {
	if c.OnBlobStart < 0 || c.OnBlobEnd > int64(len(f.data)) {
		return nil, fmt.Errorf("chunk [%d,%d) out of blob (size %d)", c.OnBlobStart, c.OnBlobEnd, len(f.data))
	}
	slice := make([]byte, c.OnBlobEnd-c.OnBlobStart)
	copy(slice, f.data[c.OnBlobStart:c.OnBlobEnd])
...
func (c *countingBP) Fetch(ctx context.Context, desc ocispec.Descriptor, ch contentindex.ChunkRef, p contentindex.Priority) (io.ReadCloser, error) {
	c.fetchCount.Add(1)
	return c.inner.Fetch(ctx, desc, ch, p)
}
